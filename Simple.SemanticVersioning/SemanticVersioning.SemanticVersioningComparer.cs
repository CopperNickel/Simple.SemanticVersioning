...
public static class SemanticVersioningComparer {
  ...
  /// <summary>
  /// Comparer version texts
  /// </summary>
  public sealed class VersionTextComparer : IComparer<SemanticVersion>, IEqualityComparer<SemanticVersion> {
    /// <summary>
    /// Compare tow version
    /// </summary>
    /// <param name="x">Left</param>
    /// <param name="y">Right</param>
    /// <returns>Negative if x less than y, 0 if x equals y, positive if x greater than y</returns>
    public int Compare(SemanticVersion? x, SemanticVersion? y) => CompareVersionText(x, y);
    ...
    /// <param name="y">Right</param>
    /// <returns>True if x equals to y</returns>
    public bool Equals(SemanticVersion? x, SemanticVersion? y) => CompareVersionText(x, y) == 0;
  ...
  /// <summary>
  /// Versions as text comparer
  /// </summary>
  public static VersionTextComparer VersionText { get; } = new();
  ...
  /// <summary>
  /// Compare Version Parts and Rank only
  /// </summary>
  /// <param name="left">Left version</param>
  /// <param name="right">Right version</param>
  /// <returns>Positive if left is less than right, 0 if they are equal, positive is left is greater than right</returns>
  ...
  public static int CompareVersionAll(SemanticVersion? left, SemanticVersion? right) {
    ...
    result = string.Compare(left.Prerelease, right.Prerelease, CultureInfo.InvariantCulture, CompareOptions.IgnoreCase | CompareOptions.NumericOrdering);

    if (result != 0)
      return result;

    result = string.Compare(left.Prerelease, right.Prerelease, CultureInfo.InvariantCulture, CompareOptions.NumericOrdering);

    if (result != 0)
      return result;

    result = string.Compare(left.Metadata, right.Metadata, CultureInfo.InvariantCulture, CompareOptions.IgnoreCase | CompareOptions.NumericOrdering);

    if (result != 0)
      return result;

    return string.Compare(left.Metadata, right.Metadata, CultureInfo.InvariantCulture, CompareOptions.NumericOrdering);
  }

  /// <summary>
  /// Compare Version As Text
  /// </summary>
  /// <param name="left">Left version</param>
  /// <param name="right">Right version</param>
  /// <returns>Positive if left is less than right, 0 if they are equal, positive is left is greater than right</returns>
  public static int CompareVersionText(SemanticVersion? left, SemanticVersion? right) {
    if (ReferenceEquals(left, right))
      return 0;
    if (left is null)
      return -1;
